...
class WajibRetribusiApiController extends Controller
{
    ...
    public function get_district(Request $request){
      $districts = Districts::where('regency_id', '=', '71')->get();
    ...
    public function get_jenis_retribusi(Request $request,$jenis){
      if($jenis == 'kota'){
          $jenis_retribusi = DB::table('jenis_retribusi')
                        ->select('id','nama','luas','tarif_kota as tarif')
                        ->get();
      }elseif($jenis == 'gampong'){
          $jenis_retribusi = DB::table('jenis_retribusi')
                        ->select('id','nama','luas','tarif_gampong as tarif')
                        ->get();
      }
    ...
    public function register(Request $request, WajibRetribusi $warr){
        ...
        $validator = Validator::make($request->all(), 
            ...
            array(  
                    ...
                    // 'nik' => 'required',
                    'nama' => 'required',
                    // 'hp'         => 'numeric|min:15|unique:wajib_retribusi,hp,',
                    ...
                    'username'         => 'required',
                    'password'         => 'required',
                    // 'email'         => 'email|unique:wajib_retribusi,email,',
                    // 'ktp'         => 'required',
      ...
      $validator = Validator::make($request->all(), 
        ...
        if ($validator->fails()) {
          ...
          if (Input::file('imagedua')) {
            ...
            $image = $request->file('imagedua');
            $input['imagename'] =  $pwbr.'imagedua'.date('ymdhis').'.'.$image->getClientOriginalExtension();
            $destinationPath = ('uploads/wrdetail');
            $img = Image::make($image->getRealPath());
            $img->save(public_path($destinationPath.'/'.$input['imagename']));
          ...
          if (Input::file('imagetiga')) {
            ...
            $image = $request->file('imagetiga');
            $input['imagename'] =  $pwbr.'imagetiga'.date('ymdhis').'.'.$image->getClientOriginalExtension();
            $destinationPath = ('uploads/wrdetail');
            $img = Image::make($image->getRealPath());
            $img->save(public_path($destinationPath.'/'.$input['imagename']));
    ...
    public function editpasword(Request $request,$id){
        ...
        if ($validator->fails()) {
            ...
            $warr->password = $encrypted_password;
            $warr->salt = $salt;
    ...
    public function get_resi(Request $request,$id){
      $validator = Validator::make($request->all(),
               array(
                  'token' => 'required'
                      ), array(
    ...
    public function get_resi_detail(Request $request,$id_pembayaran){
      try {
