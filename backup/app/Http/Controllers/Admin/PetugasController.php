    ...
    public function data_petugas(Request $request)
    {
        ...
        if (empty($request->input('search.value'))) {
            $services = DB::table('petugas as a')
                ->select('a.id', 'a.id_koordinator','a.nik','a.nama','a.hp','a.username','a.email','a.image','a.created_at','a.updated_at','a.is_active','b.nama as namakoordinator','c.nama as namazona')
        ...
        if (!empty($services)) {
            ...
            foreach ($services as $service) {
                ...
                if($service->is_active != 1){
                    $je = "<button class='btn btn-danger btn-shadowed popover-hover activee btn-xs' a='".encrypt($service->id)."' b='$service->nama' data-container='body' data-toggle='tooltip' data-placement='left' data-content='Aktifkan'><i class='icon-power-switch'></i> Tidak Aktif</button>";
                }else{
