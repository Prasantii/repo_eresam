...
<?php $i = 1;$u = 1; ?>
    ...
    <div class="app-checkbox success"> 
        <label><input id="id_districts[<?php echo $u++; ?>]" type="checkbox" name="data[<?php echo $i++; ?>][districts]" value="{{$men->id}}" <?php Helperss::check_zona($men->id,$zona->id); ?>> {{$men->name}}<span></span></label> 
    </div>
	@endforeach
</div>